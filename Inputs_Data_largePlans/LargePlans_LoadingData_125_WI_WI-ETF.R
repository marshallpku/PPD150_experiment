...
#ActivesSched %>% filter(age == 56, yos >=30)
...
SalaryGrowthSched <- data.frame(yos = 0:54) %>% 
  ...
  mutate(grate = ifelse(yos > 29, grate[yos == 29], grate),
...
if(TermRatesType != "Matrix"){
  ...
  df_TermRatesSched_singleCol <- splong(df_TermRatesSched_singleCol, "age", 20:64)
  ...
  df_TermRatesSched_singleCol %<>%
    mutate(termrate = ifelse(age < 25, termrate[age == 25], termrate),
           termrate = ifelse(age > 60, termrate[age == 60], termrate)) # plan specific
}
