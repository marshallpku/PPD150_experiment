...
SalaryGrowthSched <- data.frame(yos = 0:54) %>% 
  ...
  # mutate(grate = ifelse(age > 64, grate[age == 64], grate),
...
#   df_TermRatesSched_singleCol <- splong(df_TermRatesSched_singleCol, "yos", 0:30)
# 
#   # Merging Data
#   df_TermRatesSched_singleCol <-
...
#            termrate = ifelse(yos > 30, termrate[yos == 30], termrate)) # plan specific
# }
# 
# 
# ## Low YOS values
...
df_RetRatesSched_raw %<>%
  ...
  mutate(retrate = ifelse(retrate >= 0, retrate, 0),
         retrate = ifelse(age < 50, 0, retrate),
         retrate = ifelse(age > 70, retrate[age == 70], retrate)) %>%
...
DisbRatesSched <-
  ...
  mutate(  disbrate = ifelse(age < 20, disbrate[age == 20], disbrate),
           disbrate = ifelse(age > 55, disbrate[age == 55], disbrate),
           planname      = planname,
           plantype      = plantype,
