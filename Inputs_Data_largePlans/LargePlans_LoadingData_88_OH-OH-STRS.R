...
#          termrate = ifelse(age > 64, termrate[age == 64], termrate),
#          termrate = ifelse(termrate >= 0, termrate, 0)) %>%
#   ungroup %>% 
#   splong("yos", 0:54) %>% 
#   mutate(
#     termrate = ifelse(yos <= 4, termrate[yos == 2], termrate),
#     termrate = ifelse(yos >= 5 & yos < 9, termrate[yos == 7], termrate),
...
if(TermRatesType != "Matrix"){
  ...
  df_TermRatesSched_singleCol <- splong(df_TermRatesSched_singleCol, "age", 20:65)
...
df_RetRatesSched_raw %<>%
  ...
  mutate(retrate = ifelse(retrate >= 0, retrate, 0),
         retrate = ifelse(age < 52, 0, retrate),
         retrate = ifelse(age > 75, retrate[age == 75], retrate)) %>%
  ...
  mutate(retrate = ifelse(yos < 25,  retrate[yos == 22], retrate),
         retrate = ifelse(yos >= 25 & yos <= 29 , retrate[yos == 27], retrate),
         retrate = ifelse(yos >= 30 & yos <= 34 , retrate[yos == 32], retrate),
         retrate = ifelse(yos >= 35, retrate[yos == 37], retrate),
...
DisbRatesSched <-
  ...
  mutate(  disbrate = ifelse(age < 20, disbrate[age == 20], disbrate),
           disbrate = ifelse(age > 65, disbrate[age == 65], disbrate),
           planname      = planname,
           plantype      = plantype,
