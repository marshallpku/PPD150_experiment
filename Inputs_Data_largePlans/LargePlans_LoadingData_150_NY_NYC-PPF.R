...
# retirees already spread (72_NJ)
# RetireeSched <-
# read_ExcelRange(paste0(data_dir, planname, ".xlsx"), "RetireesSched") %>%
#   mutate(planname = planname, plantype = plantype, fillin = TRUE) %>%
#   select(planname, plantype, fillin, age, nretirees, benefit)
# RetireeSched

# No data (10 CA, 77 NY teachers)
# RetireeSched <-
#   data.frame(age = 20:110) %>%
#   mutate(
#     nretirees = NA,
#     benefit = NA,
#     planname = planname, plantype = plantype, fillin = TRUE) %>%
#   select(planname, plantype, fillin, age, nretirees, benefit)
# RetireeSched
...
SalaryGrowthSched <- data.frame(yos = 0:54) %>% 
  ...
  mutate(grate = ifelse(yos > 45, grate[yos == 45], grate),
         grate = ifelse(yos < 0,  grate[yos == 0], grate)) %>%
  # mutate(grate = ifelse(age > 70, grate[age == 70], grate),
  #        grate = ifelse(age < 20, grate[age == 20], grate)) %>%
...
#   splong("age", 20:74) %>%
#   group_by(yos) %>%
#   mutate(termrate = ifelse(age < 20, termrate[age == 20], termrate),
#          termrate = ifelse(age > 54, termrate[age == 54], termrate),
#          termrate = ifelse(termrate >= 0, termrate, 0)) %>%
#   ungroup %>%
#   splong("yos", 0:54) %>%
#   mutate(
#     termrate = ifelse(yos < 0,  termrate[yos == 0],  termrate),
#     termrate = ifelse(yos > 10, termrate[yos == 10], termrate))
# 
# df_TermRatesSched
# 
...
#   group_by(age) %>%
#   mutate(# termrate = ifelse(yos <= 9, termrate, termrate[yos == 9]),
#          planname      = planname,
#          plantype      = plantype,
...
#TermRatesSched$termrate %>% is.na %>% sum
...
if(TermRatesType != "Matrix"){
  ...
  df_TermRatesSched_singleCol <- splong(df_TermRatesSched_singleCol, "yos", 0:54)
  # df_TermRatesSched_singleCol <- splong(df_TermRatesSched_singleCol, "age", 20:74)
  ...
  df_TermRatesSched_singleCol %<>%
    # mutate(termrate = ifelse(age < 20,  termrate[age == 20], termrate),
    #        termrate = ifelse(age > 65,  termrate[age == 65], termrate)) # plan specific
    mutate(termrate = ifelse(yos <  0,  termrate[yos ==  0], termrate),
...
#          retrate = ifelse(yos == 15 & age < 55, 0, retrate),
#          retrate = ifelse(yos == 15 & age > 61, retrate[age == 61], retrate),
#          retrate = ifelse(yos == 45 & age < 55, 0, retrate),
#          retrate = ifelse(yos == 45 & age > 76, retrate[age == 76], retrate)) %>%
# 
#   ungroup %>%
#   splong("yos", 0:56) %>%
#   group_by(age) %>%
#   mutate(retrate = ifelse(yos < 30,  retrate[yos == 15], retrate),
#          retrate = ifelse(yos >= 30, retrate[yos == 45], retrate),
...
# 
# RetRatesSched <-
#   expand.grid(age = 20:76, ea = 20:76) %>%
#   filter(age >= ea) %>%
#   mutate(planname = planname,
#          plantype = plantype,
#          RetRatesType = RetRatesType,
...
#   mutate(retrate = na2zero(retrate)) %>%
#   select(planname, plantype, RetRatesType, ea, age, yos, retrate)
# 
# RetRatesSched %>% filter(ea == 40)
#**********************************************************************************************
...
if(RetRatesType != "Matrix"){
  ...
  df_RetRatesSched_singleCol %<>%
    # splong("yos", 20:54)
    splong("age", 20:74) %>%
    mutate(retrate = ifelse(age < 40, 0, retrate),
           retrate = ifelse(age > 63, retrate[age == 63], retrate))
  }
...
DisbRatesSched <-
  ...
  mutate(  disbrate = ifelse(age < 20, disbrate[age == 20], disbrate),
           disbrate = ifelse(age > 60, disbrate[age == 60], disbrate),
           planname      = planname,
           plantype      = plantype,
