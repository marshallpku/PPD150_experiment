...
results_allPlans1 <- results_allPlans #%>% filter(ppd_id %in% ppd_id_largePlans)
...
results_sumPPD <- 
  ...
  results_allPlans1 %<>% bind_rows(results_allPlans1, results_sumPPD)
  

  riskMeasure <- results_allPlans1  %>% 
    #results_sumPPD %>% 
  ...
  riskMeasure %>% filter(ppd_id == ppd_id_largePlans[1])
  riskMeasure %>% filter(ppd_id == ppd_id_largePlans[3])
...
# 
# 
x <- results_allPlans %>% filter(sim == 1, ppd_id == 83) %>%
  mutate(FR_MA     = 100 * MA / AL,
         ERC_PR    = 100 * ERC / PR) %>%
