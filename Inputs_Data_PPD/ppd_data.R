...
# write_csv(df_trim, paste0("./PPDvars/ppdtrim_for_Yimeng_", filedate, ".csv"))
...
PPD_data <- 
  ...
  select(ppd_id,
         ...
         plantype       = planf, 
         AL_ppd         = al,
         AL_ppd_src     = al_source,
         payroll_ppd    = payroll,
         MAV_ppd        = assets.mv,
         AAV_ppd = assets.av,
         UAAL_ppd       = uaal,
         init_nactives_ppd  = actives,
         init_nretirees_ppd = beneficiaries,
         B_ppd          = benefits, 
         B_avg_ppd_ppd      =  avgbenefit.ppd,
         B_avg_best_ppd     =  avgbenefit_best,
         B_avg_ppd          =  avgbenefit,
         i_ppd          =  discount,
         infl_ppd       =  infl.ppd,
         EEC_ppd        =  eec,
         salary_avg_ppd     =  avgsalary,
         
         
         EEC_rate_ppd_calc = eecRate.ppd_calc,
         
         asset_year_ppd = assetperiod,
         amort_openclosed_ppd = openclosedf,
         amort_pctdol_ppd     = pctdollf,
         amort_year_ppd     =  amortperiod,
         
         
         payroll_ppd_src = payroll_source,
         MAV_ppd_src =  assets.mv_source,
         AAV_ppd_src =  assets.av_source,
         UAAL_src    =  uaal_source,
         init_nactives_ppd_src  = actives_source,
         init_nretirees_ppd_src = beneficiaries_source, 
         B_ppd_src   = benefits_source,
         B_avg_ppd_ppd_src = avgbenefit.ppd_source,
         discount_src  = discount_source,
         infl_ppd_src  = infl.ppd_source,
         EEC_ppd_src   = eec_source,
         asset_year_ppd_src = assetperiod_source,
         amort_openclosed_ppd_src = openclosedf_source,
         amort_pctdol_ppd_src = pctdollf_source,
         amort_year_ppd_src   = amortperiod_source) %>% 
  mutate(plantype = str_extract(plantype, "General|Teacher|Safety") %>% tolower,
         year_AL  = str_extract(AL_ppd_src, "\\d+"),
         FR_MAV = MAV_ppd / AL_ppd,
         FR_AAV = AAV_ppd / AL_ppd,
         amort_pctdol_ppd = ifelse(amort_pctdol_ppd == "levpercent", "cp",
                                   ifelse(amort_pctdol_ppd == "levdollar", "cd", NA))
         ) %>% 
  mutate_at(vars(AL_ppd, payroll_ppd, MAV_ppd, AAV_ppd, UAAL_ppd, B_ppd, B_avg_ppd_ppd, B_avg_best_ppd, B_avg_ppd, EEC_ppd, salary_avg_ppd), funs(.*1000)) %>% 
